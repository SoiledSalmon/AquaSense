...
export default function AdminDashboard({ user }: { user: User }) {
  const [activeTab, setActiveTab] = useState<
    "overview" | "users" | "readings" | "alerts" | "ml" | "status"
  >("overview");
  ...
  const fetchAlerts = async () => {
    try {
      ...
      const res = await getAdminAlerts({
        severity:
          alertsSeverityFilter !== "all" ? alertsSeverityFilter : undefined,
        isAcknowledged:
          alertsAckFilter === "ack"
            ? true
            : alertsAckFilter === "unack"
              ? false
              : undefined,
        page: alertsPage,
        limit: itemsPerPage,
  ...
  const fetchML = async () => {
    try {
      ...
      const res = await getAdminML({
        ...
        isAnomaly:
          mlAnomalyFilter === "anomaly"
            ? true
            : mlAnomalyFilter === "normal"
              ? false
              : undefined,
        page: mlPage,
        limit: itemsPerPage,
  ...
  const getShapChartData = () => {
    ...
    return [
      {
        name: "pH Contribution",
        value: Number((phSum / count).toFixed(4)),
        fill: "#3b82f6",
      },
      {
        name: "TDS Contribution",
        value: Number((tdsSum / count).toFixed(4)),
        fill: "#06b6d4",
      },
      {
        name: "Turbidity Contribution",
        value: Number((turbSum / count).toFixed(4)),
        fill: "#a855f7",
      },
    ];
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Global monitoring panel for user profiles, real-time channels, alert
            queues, and ML performance.
          </p>
        </div>
      ...
      <div className="flex flex-wrap border-b border-slate-800">
        {[
          ...
          return (
            ...
            >
              <Icon
                className={`h-4 w-4 ${active ? "text-purple-400" : "text-slate-400"}`}
              />
              {t.name}
            </button>
          );
            ...
            {[
              {
                ...
                subtitle: "Configured channel IDs",
                icon: Users,
                color: "text-blue-400",
              },
              {
                title: "Total Telemetry Items",
                value: stats.total_readings_count.toLocaleString(),
                subtitle: "Ingested over MQTT",
                icon: Droplets,
                color: "text-cyan-400",
              },
              {
                title: "Unsafe Incidents",
                value: stats.unsafe_events_count,
                subtitle: "WQI safety events logged",
                icon: AlertTriangle,
                color: "text-rose-500",
              },
              {
                title: "Average WQI",
                value: stats.average_wqi,
                subtitle: "Global water quality average",
                icon: TrendingUp,
                color: getWqiColor(stats.average_wqi),
              },
            ].map((card, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 backdrop-blur-md hover:border-slate-700/80 transition-all duration-300"
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div
                      className={`p-2.5 rounded-lg bg-slate-950 border border-slate-800 ${card.color}`}
                    >
              ...
              <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 backdrop-blur-md">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center bg-slate-950/60 p-3 rounded-lg border border-slate-800">
                    ...
                    >
                      {stats.system_status?.database?.toUpperCase() ||
                        "UNKNOWN"}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center bg-slate-950/60 p-3 rounded-lg border border-slate-800">
                    ...
                    >
                      {stats.system_status?.mqtt_subscriber?.toUpperCase() ||
                        "UNKNOWN"}
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 backdrop-blur-md">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-950/60 p-4 rounded-lg border border-slate-800 text-center">
                    ...
                    <p className="text-2xl font-extrabold text-slate-200 mt-2">
                      {stats.alert_frequency?.last_24h}
                    </p>
                  </div>
                  <div className="bg-slate-950/60 p-4 rounded-lg border border-slate-800 text-center">
                    <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">
            ...
            <div className="flex gap-4 w-full md:w-auto">
              <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 w-full md:w-auto">
                ...
                >
                  ...
                  <option value="user" className="bg-slate-950 text-slate-300">
                    User Only
                  </option>
                  <option value="admin" className="bg-slate-950 text-slate-300">
                    Admin Only
                  </option>
                </select>
              </div>
                ...
                <div className="flex gap-3 text-rose-500">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mt-2">
                      This action will immediately delete the user from Supabase
                      authentication databases. All telemetry records, warnings,
                      and anomaly history will be deleted.
                    </p>
                  </div>
                </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-950/60 border-b border-slate-800">
                    <th className="p-4 text-xs uppercase font-bold text-slate-400 tracking-wider">
                      User Details
                    </th>
                    <th className="p-4 text-xs uppercase font-bold text-slate-400 tracking-wider">
                      Role
                    ...
                    <th className="p-4 text-xs text-right uppercase font-bold text-slate-400 tracking-wider">
                      Actions
                    </th>
                  </tr>
                </thead>
                        ...
                        <td className="p-4 text-right">
                          <div className="flex gap-2 justify-end">
                            <button
                              ...
                              disabled={
                                updatingUserId !== null || usr.id === user.id
                              }
          ...
          {usersCount > itemsPerPage && (
            <div className="flex items-center justify-between border-t border-slate-800 pt-4">
              <span className="text-xs text-slate-500">
                ...
                {Math.min(usersPage * itemsPerPage, usersCount)} of {usersCount}{" "}
                users
              </span>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setUsersPage((p) =>
                      Math.min(p + 1, Math.ceil(usersCount / itemsPerPage)),
                    )
                  }
      ...
      {activeTab === "readings" && (
        <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 md:p-6 backdrop-blur-md space-y-6">
            ...
            <div className="flex flex-wrap gap-3 w-full md:w-auto">
              <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5">
                ...
                >
                  ...
                  <option value="safe" className="bg-slate-950 text-slate-300">
                    Safe Only
                  </option>
                  <option
                    value="borderline"
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {readingsList.length > 0 ? (
                    readingsList.map((r) => (
                      <tr
                        key={r.id}
                        className="hover:bg-slate-900/10 transition-all duration-200"
                      >
                        <td className="p-4 text-sm font-mono text-slate-300">
                        ...
                        <td className="p-4 text-sm text-slate-200 font-mono">
                          {r.ph !== null ? Number(r.ph).toFixed(2) : "-"}
                        </td>
                        <td className="p-4 text-sm text-slate-200 font-mono">
                          {r.tds !== null
                            ? `${Number(r.tds).toFixed(0)} ppm`
                            : "-"}
                        </td>
                        <td className="p-4 text-sm text-slate-200 font-mono">
                          {r.turbidity !== null
                            ? `${Number(r.turbidity).toFixed(1)} NTU`
                            : "-"}
                        </td>
                        <td
                          className={`p-4 text-sm font-bold font-mono ${getWqiColor(r.wqi_score)}`}
                        >
                          {r.wqi_score !== null
                            ? Number(r.wqi_score).toFixed(1)
                            : "-"}
                        </td>
                        <td className="p-4">
                          <span
                            className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${getWqiBg(r.wqi_score)}`}
                          >
                            {r.label || "unknown"}
                          </span>
                        </td>
          ...
          {readingsCount > itemsPerPage && (
            <div className="flex items-center justify-between border-t border-slate-800 pt-4">
              <span className="text-xs text-slate-500">
                Showing {(readingsPage - 1) * itemsPerPage + 1} to{" "}
                {Math.min(readingsPage * itemsPerPage, readingsCount)} of{" "}
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setReadingsPage((p) =>
                      Math.min(p + 1, Math.ceil(readingsCount / itemsPerPage)),
                    )
                  }
                  disabled={
                    readingsPage === Math.ceil(readingsCount / itemsPerPage)
                  }
                  className="p-1.5 border border-slate-800 bg-slate-950 text-slate-400 rounded-lg hover:text-white disabled:opacity-50 transition-all duration-200"
                >
                  <ChevronRight className="h-4.5 w-4.5" />
            ...
            <div className="flex flex-wrap gap-3 w-full md:w-auto">
              <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5">
                ...
                >
                  ...
                  <option value="info" className="bg-slate-950 text-slate-300">
                    Info Only
                  </option>
                  <option
                    value="warning"
              ...
              <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5">
                ...
                >
                  ...
                  <option value="unack" className="bg-slate-950 text-slate-300">
                    Unacknowledged
                  </option>
                  <option value="ack" className="bg-slate-950 text-slate-300">
                    Acknowledged
                  </option>
                </select>
              </div>
                    ...
                    alertsList.map((a) => (
                      ...
                      >
                        ...
                        <td className="p-4 space-y-1">
                          <div className="flex gap-2">
                            <span
                              className={`text-[10px] font-bold uppercase px-1.5 py-0.25 rounded border ${
                                a.is_acknowledged
                                  ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400"
                                  : "bg-rose-500/10 border-rose-500/20 text-rose-400"
                              }`}
          ...
          {alertsCount > itemsPerPage && (
            <div className="flex items-center justify-between border-t border-slate-800 pt-4">
              <span className="text-xs text-slate-500">
                ...
                {Math.min(alertsPage * itemsPerPage, alertsCount)} of{" "}
                {alertsCount} alerts
              </span>
              <div className="flex gap-2">
                <button
                ...
                <button
                  onClick={() =>
                    setAlertsPage((p) =>
                      Math.min(p + 1, Math.ceil(alertsCount / itemsPerPage)),
                    )
                  }
                  disabled={
                    alertsPage === Math.ceil(alertsCount / itemsPerPage)
                  }
                  className="p-1.5 border border-slate-800 bg-slate-950 text-slate-400 rounded-lg hover:text-white disabled:opacity-50 transition-all duration-200"
                >
                  <ChevronRight className="h-4.5 w-4.5" />
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-slate-900/40 border border-slate-800 rounded-xl p-5 md:p-6 backdrop-blur-md">
              ...
              <p className="text-slate-400 text-xs mt-1">
                Relative importance of pH, TDS, and Turbidity values in deciding
                anomaly classifications for recent runs.
              </p>
            ...
            {stats && (
              <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 backdrop-blur-md flex flex-col justify-between">
                ...
                <div className="text-[11px] text-slate-500 bg-slate-950 border border-slate-800 p-2.5 rounded-lg mt-4 flex gap-1.5 items-start">
                  ...
                  <span>
                    Isolation Forest model assesses anomalies; XGBoost predicts
                    WQI scores; SHAP generates feature attributions.
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 md:p-6 backdrop-blur-md space-y-6">
              ...
              <div className="flex gap-3 w-full md:w-auto">
                <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5">
                  ...
                  >
                    ...
                    <option value="low" className="bg-slate-950 text-slate-300">
                      Low Risk Only
                    </option>
                    <option
                      value="medium"
                    ...
                    {mlList.length > 0 ? (
                      mlList.map((p) => (
                        ...
                        >
                          ...
                          <td className="p-4 text-xs font-mono text-slate-300">
                            pH:{" "}
                            {p.ph_smoothed !== null
                              ? Number(p.ph_smoothed).toFixed(2)
                              : "-"}{" "}
                            | TDS:{" "}
                            {p.tds_smoothed !== null
                              ? `${Number(p.tds_smoothed).toFixed(0)} ppm`
                              : "-"}{" "}
                            | Turb:{" "}
                            {p.turb_smoothed !== null
                              ? `${Number(p.turb_smoothed).toFixed(1)} NTU`
                              : "-"}
                          </td>
                          <td className="p-4 text-sm font-mono text-slate-300">
                            <span
                            ...
                            >
                              {p.anomaly_score !== null
                                ? Number(p.anomaly_score).toFixed(4)
                                : "-"}
                            </span>
                            {p.is_anomaly && (
                          ...
                          <td className="p-4 text-xs font-mono text-slate-400">
                            pH:{" "}
                            {p.shap_ph !== null
                              ? Number(p.shap_ph).toFixed(4)
                              : "-"}{" "}
                            | TDS:{" "}
                            {p.shap_tds !== null
                              ? Number(p.shap_tds).toFixed(4)
                              : "-"}{" "}
                            | Turb:{" "}
                            {p.shap_turbidity !== null
                              ? Number(p.shap_turbidity).toFixed(4)
                              : "-"}
                          </td>
                        </tr>
                      ))
                      ...
                      <tr>
                        <td
                          ...
                          className="p-6 text-center text-slate-500 text-sm"
                        >
                          No ML prediction records found matching the filter
                          parameters
                        </td>
                      </tr>
                    )}
            ...
            {mlCount > itemsPerPage && (
              <div className="flex items-center justify-between border-t border-slate-800 pt-4">
                <span className="text-xs text-slate-500">
                  Showing {(mlPage - 1) * itemsPerPage + 1} to{" "}
                  {Math.min(mlPage * itemsPerPage, mlCount)} of {mlCount}{" "}
                  predictions
                </span>
                <div className="flex gap-2">
                  <button
                  ...
                  <button
                    onClick={() =>
                      setMlPage((p) =>
                        Math.min(p + 1, Math.ceil(mlCount / itemsPerPage)),
                      )
                    }
      ...
      {activeTab === "status" && stats && (
        <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 md:p-6 backdrop-blur-md space-y-6">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1">
              Real-time monitoring stats and connectivity diagnostics for the
              database clusters and ingestion subscriber queues.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-slate-300">
                    ThingSpeak MQTT Ingestion Service
                  </span>
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono">
                    {stats.system_status?.mqtt_subscriber?.toUpperCase() ||
                      "UNKNOWN"}
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-bold uppercase tracking-wider text-slate-500">
                Ingestion Inflows (ThingSpeak MQTT Connection)
              </h4>
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3 text-sm text-slate-300">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-xs font-bold text-blue-400">
                    {stats.active_users_count} channels
                  </span>
                </div>
                <div className="flex justify-between">
