...
from app.services.alert_engine import AlertSeverityClassifier, NotificationFormatter, AlertRuleEngine
...
def test_severity_classification():
    ...
    assert AlertSeverityClassifier.classify("ml_recommendation", 0.0, {"risk_level": "high", "label": "unsafe"}) == "critical"
    assert AlertSeverityClassifier.classify("ml_recommendation", 0.0, {"risk_level": "medium", "label": "borderline"}) == "warning"
    assert AlertSeverityClassifier.classify("ml_recommendation", 0.0, {"risk_level": "low", "label": "safe"}) == "info"
...
async def test_alert_service_cooldown_deduplication():
    ...
    mock_repo = MagicMock(spec=AlertRepository)
    ...
    AlertRepository.create_alert = AsyncMock(return_value={"id": "new-alert-uuid", "category": "ph"})
    ...
    try:
        ...
        alerts = await service.process_alerts(reading, mock_client, cooldown_seconds=600)
        assert len(alerts) == 1
        ...
        AlertRepository.get_latest_alert_by_category = AsyncMock(return_value=previous_alert)
        
        alerts_suppressed = await service.process_alerts(reading, mock_client, cooldown_seconds=600)
        ...
        AlertRepository.get_latest_alert_by_category = AsyncMock(return_value=previous_alert_old)
        
        alerts_new = await service.process_alerts(reading, mock_client, cooldown_seconds=600)
