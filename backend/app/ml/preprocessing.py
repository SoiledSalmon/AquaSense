...
VALID_RANGES = {
    "ph": (0.0, 14.0),
    "tds": (0.0, 5000.0),
    "turbidity": (0.0, 5000.0)
}


def validate_and_impute_reading(reading: Dict[str, Any], history_defaults: Optional[Dict[str, float]] = None) -> Dict[str, float]:
    ...
    for feature in ["ph", "tds", "turbidity"]:
        ...
        if val is None:
            ...
            logger.info("preprocessing_missing_value_imputed", feature=feature, default_value=defaults[feature])
            continue
        ...
        try:
            ...
            imputed[feature] = defaults[feature]
            logger.warning("preprocessing_invalid_type_imputed", feature=feature, raw_value=val, default_value=defaults[feature])
            continue
        ...
        if min_val <= val_float <= max_val:
            ...
            imputed[feature] = defaults[feature]
            logger.warning("preprocessing_out_of_range_imputed", feature=feature, raw_value=val_float, default_value=defaults[feature])
