...
class MLPipelineService:
    ...
    async def process(
        self, reading: Dict[str, Any], repo: ReadingsRepository
    ) -> Dict[str, Any]:
        ...
        if not user_id or not reading_id:
            logger.error(
                "ml_pipeline_missing_identifiers",
        ...
        logger.info(
            "ml_pipeline_execution_started", reading_id=reading_id, user_id=user_id
        )
        ...
        try:
            ...
            label, score, probs = await asyncio.to_thread(
                run_xgb_inference, smoothed_features
            )
            result["label"] = label

            # 5. Isolation Forest Anomaly Detection (on smoothed features)
            is_anomaly, anomaly_score = await asyncio.to_thread(
                detect_anomaly, smoothed_features
            )

            # 6. SHAP Explanations (for the predicted class)
            shap_values = await asyncio.to_thread(
                compute_exact_shap, smoothed_features, label
            )
            ...
            try:
                ...
                logger.error(
                    "ml_pipeline_readings_update_failed",
                    reading_id=reading_id,
                    error=str(db_err),
                )
            ...
            try:
                ...
                logger.error(
                    "ml_pipeline_ml_results_insert_failed",
                    reading_id=reading_id,
                    error=str(db_err),
                )
            ...
            try:
                generated_alerts = await alert_service.process_alerts(
                    result, repo._client
                )
                if generated_alerts:
                ...
                logger.error(
                    "ml_pipeline_alerts_processing_failed",
                    reading_id=reading_id,
                    error=str(alert_err),
                )

            logger.info(
                "ml_pipeline_execution_success",
                reading_id=reading_id,
                label=label,
                wqi_score=wqi_score,
            )
