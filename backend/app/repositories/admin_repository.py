...
class AdminRepository:
    ...
    async def get_users(
        ...
        offset = (page - 1) * limit
        query = query.order(sort_by, desc=(order == "desc")).range(
    ...
    async def get_users_count(
        self, search: Optional[str] = None, role: Optional[str] = None
    ) -> int:
    ...
    async def get_readings_count(
        self, user_id: Optional[str] = None, label: Optional[str] = None
    ) -> int:
    ...
    async def get_stats_fallback(self) -> Dict[str, Any]:
        ...
        readings_cnt_res = (
            await self._client.table("readings")
        ...
        wqi_scores = [
            r["wqi_score"]
            for r in (wqi_res.data or [])
            if r.get("wqi_score") is not None
        ]
        ...
        alerts_24h = (
            await self._client.table("alerts")
            .select("id", count="exact")
        ...
        # 5. ML distribution
        anomaly_count = (
            await self._client.table("ml_results")
            .select("id", count="exact")
            .eq("is_anomaly", True)
            .execute()
        ).count or 0
        ...
        trends_res = (
            await self._client.table("readings_daily")
            .select("bucket, avg_wqi")
        ...
        return {
            ...
            "alert_frequency": {"last_24h": alerts_24h, "last_7d": alerts_7d},
