...
def load_models(force_retrain: bool = False):
    ...
    need_train = (
        force_retrain
        or not os.path.exists(XGB_PATH)
        or not os.path.exists(IFOREST_PATH)
    )

    if need_train:
        logger.info(
            "ml_models_not_found_or_retrain_requested",
            force_retrain=force_retrain,
            sklearn_version=sklearn.__version__,
        )
        # Delete stale model files to avoid loading corrupt/incompatible artifacts
        for path in (XGB_PATH, IFOREST_PATH):
            if os.path.exists(path):
                os.remove(path)
                logger.info("ml_stale_model_deleted", path=path)
    ...
    try:
        ...
        _xgb_model = clf
        logger.info("ml_xgb_model_loaded", path=XGB_PATH, xgb_version=xgb.__version__)

        # Load Isolation Forest
        _iforest_model = joblib.load(IFOREST_PATH)
        logger.info(
            "ml_iforest_model_loaded",
            path=IFOREST_PATH,
            sklearn_version=sklearn.__version__,
        )
