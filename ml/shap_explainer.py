...
import pandas as pd
import structlog
from .loader import get_xgb_model

logger = structlog.get_logger()

# Default baseline / background values representing optimal water quality
DEFAULT_BASELINE = {"ph": 7.0, "tds": 250.0, "turbidity": 1.0}

LABEL_TO_IDX = {"safe": 0, "borderline": 1, "unsafe": 2}
...
def compute_exact_shap(
    features: Dict[str, float], predicted_label: str, baseline: Dict[str, float] = None
) -> Dict[str, float]:
    ...
    f_ph_turb = evaluate_coalition(
        ["ph", "turbidity"], model_features, bg, model, class_idx
    )
    f_tds_turb = evaluate_coalition(
        ["tds", "turbidity"], model_features, bg, model, class_idx
    )
    # Full set
    f_all = evaluate_coalition(
        ["ph", "tds", "turbidity"], model_features, bg, model, class_idx
    )
