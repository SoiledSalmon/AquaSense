...
def admin_service(mock_supabase, mock_supabase_admin, monkeypatch):
    mock_repo = MagicMock()
    monkeypatch.setattr(
        "app.services.admin_service.AdminRepository", lambda client: mock_repo
    )
...
async def test_update_user_role_success(admin_service, mock_supabase_admin):
    ...
    mock_repo.get_user_by_id = AsyncMock(
        return_value={"id": "user-123", "role": "user"}
    )
    mock_repo.update_user_role = AsyncMock(
        return_value={"id": "user-123", "role": "admin"}
    )
...
async def test_admin_repository_get_users(mock_supabase):
    ...
    mock_execute.execute = AsyncMock(
        return_value=MagicMock(data=[{"id": "u1", "email": "test@test.com"}])
    )
