...
def mock_supabase_admin():
    admin = MagicMock()
    admin.auth.sign_up = AsyncMock()
    admin.auth.admin.update_user_by_id = AsyncMock()
    admin.auth.sign_in_with_password = AsyncMock()
...
def auth_service(mock_supabase, mock_supabase_admin, monkeypatch):
    mock_repo = MagicMock()
    monkeypatch.setattr(
        "app.services.auth_service.UserRepository", lambda client: mock_repo
    )
...
async def test_logout_success(auth_service, mock_supabase_admin):
    ...
    mock_supabase_admin.auth.admin.sign_out.assert_called_once_with(
        "mock-uuid-123", scope="global"
    )
...
@pytest.mark.asyncio
async def test_signup_profile_creation_failure_rollback(
    auth_service, mock_supabase_admin
):
    service, mock_repo = auth_service
    ...
    # Mock signup success on auth client
    mock_supabase_admin.auth.sign_up.return_value = auth_response_mock
    ...
    # Mock create_user to fail and raise ProfileCreationError
    from app.core.exceptions import ProfileCreationError

    mock_repo.create_user = AsyncMock(side_effect=ProfileCreationError("mock-uuid-123"))
    ...
    # Assert that delete_user was called to rollback the user
    mock_supabase_admin.auth.admin.delete_user.assert_called_once_with("mock-uuid-123")
